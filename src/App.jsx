...
const firebaseConfig = {
  apiKey: 'AIzaSyBz0UsXgQblxalS-uM7LbLGo-yBhvROs0M',
...
const AI_PLAYLISTS = [
  {
    ...
    description:
      'Các bản cover nhạc cực chất được tạo bởi AI định dạng video ngắn.',
    thumbnail:
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const iconUrl =
      'https://lh3.googleusercontent.com/d/1RjUhWb2asNqhetIBybpZJ8EVCqYQDdo5';
    ...
    const initAuth = async () => {
      try {
        ...
        if (
          ...
          setAuthError(
            'Bạn chưa kích hoạt tính năng Đăng nhập Ẩn danh (Anonymous) trên Firebase.'
          );
        } else {
  ...
  const handleDeleteCategory = async (id, catName) => {
    ...
    setConfirmDialog({
      ...
      message:
        'Ứng dụng trong danh mục này sẽ không bị xoá, nhưng bạn nên phân loại lại chúng.',
      onConfirm: async () => {
        try {
  ...
  const getCategoryIcon = (cat) => {
    switch (cat) {
      ...
      default:
        return <LayoutGrid size={16} className="mr-1" />;
    }
  };
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg text-center border-t-4 border-red-500">
          ...
          <h1 className="text-2xl font-bold text-gray-800 mb-2">
            Lỗi Quyền Truy Cập Firebase
          </h1>
          <p className="text-gray-600 mb-4 text-lg">
            Firebase đang chặn ứng dụng đọc dữ liệu. Bạn cần cập nhật Quy tắc
            (Rules).
          </p>
          <div className="bg-blue-50 text-blue-800 p-5 rounded-xl text-sm text-left">
            ...
            <ol className="list-decimal ml-5 mt-3 space-y-2">
              <li>
                Mở <strong>Firebase Console</strong> &gt;{' '}
                <strong>Firestore Database</strong>.
              </li>
              <li>
                Chuyển sang tab <strong>Rules (Quy tắc)</strong>.
              </li>
  ...
  match /databases/{database}/documents {
    match /{document=**} {
      allow read, write: if true;
              ...
              <li>
                Bấm nút <strong>Publish (Xuất bản)</strong>. Sau đó quay lại đây
                tải lại (F5) trang!
              </li>
            </ol>
          </div>
        ...
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-lg text-center border-t-4 border-red-500">
          ...
          <div className="mt-6 pt-5 border-t border-gray-100">
            <button
              onClick={() => {
                setAuthError('');
                setUser({ uid: 'local-test-bypass-user' });
                setLoading(true);
              }}
              className="w-full bg-gray-100 text-gray-700 font-semibold rounded-xl py-3 hover:bg-gray-200 transition active:scale-95"
            >
        ...
        <div className="bg-slate-900 relative px-5 pt-8 pb-6 rounded-b-3xl text-white sticky top-0 z-10 shadow-[0_10px_30px_-10px_rgba(6,182,212,0.4)] border-b border-cyan-500/30 overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-5 relative z-10">
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                ...
                <h1 className="text-3xl md:text-4xl font-black tracking-widest font-tech text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-400 to-indigo-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.5)] uppercase flex items-center">
                  Hoàng
                  <span className="text-cyan-50 font-light ml-2 tracking-normal">
                    Appstore
                  </span>
                </h1>
              </div>
              <div className="md:hidden">
                {isAdmin ? (
                  <button
                    ...
                    className="p-2.5 bg-slate-800 border border-cyan-500/30 text-cyan-400 rounded-full hover:bg-slate-700 hover:shadow-[0_0_15px_rgba(34,211,238,0.4)] transition"
                  >
                    <LogOut size={20} />
                  </button>
                ) : (
                  <button
                    ...
                    className="p-2.5 text-cyan-400 hover:bg-slate-800 hover:shadow-[0_0_15px_rgba(34,211,238,0.3)] rounded-full transition"
                  >
                    <UserCircle size={26} />
                  </button>
                )}
            ...
            <div className="relative flex-1 w-full md:max-w-md lg:max-w-xl group">
              <Search
                className="absolute left-4 top-3.5 text-cyan-500 group-focus-within:text-cyan-300 transition-colors"
                size={20}
              />
              <input
                type="text"
            ...
            <div className="hidden md:block">
              {isAdmin ? (
                <button
                  onClick={handleLogout}
                  title="Đăng xuất"
                  className="p-3 bg-slate-800 border border-cyan-500/30 text-cyan-400 rounded-full hover:bg-slate-700 hover:shadow-[0_0_15px_rgba(34,211,238,0.5)] transition flex items-center shadow-lg"
                >
                  <LogOut size={22} />
                </button>
              ) : (
                <button
                  ...
                  className="p-3 text-cyan-400 hover:bg-slate-800 hover:shadow-[0_0_15px_rgba(34,211,238,0.3)] border border-transparent hover:border-cyan-500/30 rounded-full transition flex items-center"
                >
                  <UserCircle size={26} />
                </button>
              )}
          ...
          <div className="flex justify-center mt-6 relative z-10">
            <div className="bg-slate-950/60 p-1.5 rounded-full border border-cyan-500/30 flex shadow-inner backdrop-blur-sm">
              <button
                ...
                className={`px-5 py-2 rounded-full font-bold text-sm flex items-center transition-all duration-300 ${
                  activeView === 'apps'
            ...
            {isAdmin && (
              <div className="mx-5 my-5 bg-slate-900 border border-cyan-500/30 rounded-xl p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4 shadow-lg relative overflow-hidden">
                ...
                <div className="flex items-center text-cyan-400 text-sm font-bold tracking-wide uppercase relative z-10">
                  <ShieldCheck
                    size={20}
                    className="mr-2 text-cyan-300 drop-shadow-[0_0_5px_rgba(34,211,238,0.8)]"
                  />
            ...
            <div className="px-5 pb-20 mt-4">
              {loading ? (
                ...
                <div className="text-center py-20 text-slate-400">
                  <LayoutGrid
                    size={48}
                    className="mx-auto mb-4 opacity-20 text-cyan-500"
                  />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredApps.map((app) => (
                    <div
                      key={app.id}
                      className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow group relative flex items-center"
                    >
                      ...
                      <div className="absolute z-50 bottom-full left-1/2 transform -translate-x-1/2 mb-4 w-72 pointer-events-none">
                        <div className="opacity-0 transform translate-y-6 scale-90 group-hover:opacity-100 group-hover:translate-y-0 group-hover:scale-100 transition-all duration-400 ease-out origin-bottom">
                          <div className="bg-slate-900/85 backdrop-blur-xl border border-cyan-400/50 rounded-2xl p-4 shadow-[0_0_25px_rgba(34,211,238,0.4)] relative">
                            <p className="whitespace-pre-wrap leading-relaxed text-[13.5px] text-cyan-50 drop-shadow-md">
                              {app.description}
                            </p>
                          ...
                          {app.iconUrl ? (
                            <img
                              ...
                              onError={(e) => {
                                ...
                                e.target.src =
                                  'https://via.placeholder.com/150?text=App';
                              }}
                            />
                          ) : (
                      ...
                      {isAdmin && (
                        <div className="flex flex-col space-y-1.5 ml-2">
                          <button
                            ...
                            className="p-1.5 text-gray-400 hover:text-blue-600 bg-gray-50 hover:bg-blue-50 rounded-md transition"
                            title="Sửa"
                          >
                            <Edit size={16} />
                          </button>
                          <button
                            ...
                            className="p-1.5 text-gray-400 hover:text-red-600 bg-gray-50 hover:bg-red-50 rounded-md transition"
                            title="Xóa"
                          >
                            <Trash2 size={16} />
                          </button>
          ...
          <div className="px-5 py-8 pb-20 animate-in fade-in duration-500">
            <div className="max-w-5xl mx-auto">
              <h2 className="text-2xl font-black text-slate-800 mb-8 flex items-center font-tech tracking-wider uppercase">
                <Youtube
                  className="text-red-500 mr-3 drop-shadow-md"
                  size={32}
                />
                Danh Sách Phát AI
              </h2>
                    ...
                    <div className="relative w-full aspect-video rounded-2xl overflow-hidden mb-5 bg-slate-900 shadow-inner">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center bg-slate-900/30 group-hover:bg-transparent transition-colors duration-300">
                        <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300 border border-white/40 shadow-[0_0_20px_rgba(255,255,255,0.4)]">
                          <Youtube
                            size={32}
                            className="text-white ml-0 fill-red-500"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="px-2">
                      <h3 className="text-xl font-bold text-slate-800 group-hover:text-cyan-600 transition-colors mb-1">
                        {pl.title}
                      </h3>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleLogin}>
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 text-white font-bold rounded-xl py-3 hover:bg-blue-700 transition active:scale-95"
              >
                Đăng Nhập
              </button>
            </form>
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm max-h-[80vh] flex flex-col shadow-2xl relative animate-in zoom-in duration-200">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
              <h2 className="text-xl font-bold flex items-center">
                <Tags size={20} className="mr-2 text-blue-600" /> Quản lý Danh
                mục
              </h2>
              <button
          ...
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in slide-in-from-bottom sm:slide-in-from-bottom-0 sm:zoom-in duration-300">
            <div className="sticky top-0 bg-white/90 backdrop-blur-md px-6 py-4 border-b border-gray-100 flex justify-between items-center z-10">
              ...
              <button
                ...
                className="p-2 bg-gray-100 rounded-full text-gray-500 hover:text-gray-800 disabled:opacity-50"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveApp} className="p-6 space-y-5">
              ...
              <div>
                ...
                {dbCategories.length === 0 ? (
                  <div className="text-sm text-orange-600 bg-orange-50 p-2 rounded-lg border border-orange-200">
                    Chưa có danh mục nào. Hãy vào "Quản lý Danh mục" để tạo
                    trước!
                  </div>
                ) : (
                  <select
              ...
              <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                <label className="block text-sm font-semibold text-gray-800 mb-2 flex items-center">
                  <LinkIcon size={16} className="mr-1.5 text-blue-600" /> Icon
                  Ứng dụng {editingId ? '' : '*'}
                </label>
                <input
                  type="url"
                  ...
                  onChange={(e) => {
                    ...
                    const driveRegex =
                      /(?:drive\.google\.com\/file\/d\/|drive\.google\.com\/open\?id=)([-_A-Za-z0-9]+)/;
                ...
                <p className="text-xs text-gray-500 mt-2 italic">
                  Dán link trực tiếp hoặc link Google Drive (hệ thống sẽ tự động
                  trích xuất).
                </p>
              ...
              <div className="pt-2 mt-2 border-t border-gray-100 flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={isUploading || dbCategories.length === 0}
                  className="flex-1 px-4 py-3 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 shadow-md transition active:scale-95 flex items-center justify-center disabled:opacity-50 disabled:bg-blue-400"
                >
                  {isUploading ? (
                    <>
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl relative animate-in zoom-in duration-200 border border-slate-200">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2.5 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition shadow-lg shadow-red-600/30"
              >
                Xóa
              </button>
            </div>
